...
@allure.title('[UI] Проверка операционной системы Android')
@allure.severity(Severity.MINOR)
@allure.feature('[UI] OperationSystemAndroid')
@allure.description('[UI] Проверка создания, удаления операционной системы Android')
class TestOperationSystemsAndroid:

    @allure.title('[UI] Добавление Android')
    ...
    @allure.title('[UI] Удаление Android')
    def test_delete_android(self):
        page = shared_vars.set_page(name="Операционные системы")
        page.fill_field_enter(locator=page.get_locator_by_name(name="Отображение"), value="999")
        page.click_button(locator=page.format_locator(page.UNIVERSAL_DIV_XPATH, div_text='Android' + ' ' + VERSION))
